  ...
  while True:
    ...
    if sm.updated['gpsLocationExternal']:
      ...
      if gps.flags % 2 == 0 or gps.accuracy > 5.:
        ...
        logs.append([datetime.datetime.utcfromtimestamp(gps.timestamp*0.001).isoformat(), str(lat), str(lng), str(gps.altitude)])
        log_count += 1
        lost_signal_count = 0
    '''
    write to log if
...
def trkpt_template(lat, lon, ele, time):
  str = ""
...
def to_gpx(logs, timestamp):
  if len(logs) > 0:
    ...
    str += "<trk>\n"
    str += "  <name>" + timestamp + "</name>"
    str += "  <trkseg>\n"
    for trkpt in logs:
      str += trkpt_template(trkpt[1], trkpt[2], trkpt[3], trkpt[0])
    str += "  </trkseg>\n"
    str += "</trk>\n"
    str += "</gpx>\n"
    try:
